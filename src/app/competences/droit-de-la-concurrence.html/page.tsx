...
const PAGE_SLUG = "competences/droit-de-la-concurrence.html";

export async function generateMetadata(): Promise<Metadata> {
  const page = await getPageBySlug(PAGE_SLUG);

  const title = page?.title ?? "Droit de la concurrence";
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${BASE_URL}/competences/droit-de-la-concurrence.html`,
    },
  };
}

export default async function DroitDeLaConcurrencePage() {
  const page = await getPageBySlug(PAGE_SLUG);

  return (
    <div className="space-y-8">
      <PageHeader
        title={page?.h1 ?? page?.title ?? "Droit de la concurrence"}
        subtitle={null}
        breadcrumb={[
          { label: "Accueil", href: "/" },
